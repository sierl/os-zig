...
pub fn allocatePage() AllocateError!u64 {
    ...
    log.debug("allocated at physical address 0x{x} or virtual address 0x{x}", .{
        vmm.physicalAddressFromDirectMap(virtual_address),
        virtual_address,
    });
...
pub fn deallocatePage(virtual_address: u64) void {
    std.debug.assert(std.mem.isAlignedGeneric(u64, vmm.physicalAddressFromDirectMap(virtual_address), page_size));
    ...
    log.debug("deallocated at physical address 0x{x} or virtual address 0x{x}", .{
        vmm.physicalAddressFromDirectMap(virtual_address),
        virtual_address,
    });
}
